...
function doGet(e) {
  ...
  if (action === "listPremios") {
    try { return apiResponse(e, listPremios(e.parameter.nombre)); }
    catch (err) { return apiResponse(e, { error: "No se pudieron listar los premios: " + err.message }); }
  }
  ...
  if (action === "deletePremio") return apiResponse(e, deletePremio(e.parameter.id, e.parameter.token));
...
function getOrCreateConfigSheet(spreadsheet) {
  ...
  headers.forEach((header, index) => { if (!sheet.getRange(1, index + 1).getValue()) sheet.getRange(1, index + 1).setValue(header); });
  return sheet;
}
    ...
    values.forEach(row => {
      if (row[0]) {
        map[row[0]] = {
          ...
          estado: row[2] || "Activo", precio: Number(row[3]) || DEFAULT_TICKET_PRICE, descripcion: row[4] || "", imagen: row[5] || ""
        };
      }
    });
  }
